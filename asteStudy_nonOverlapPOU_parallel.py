...
print("shape_parameter: ", shape_parameter)
bf = basisfunctions.Gaussian(shape_parameter)
out_mesh_list_global = []		# out_mesh_list containt of all global points solved on this rank
fr_regular_rank = []			# Contains all results from this rank
...
for dd2 in range(0,yDomainDecomposition):
	for dd1 in range(0,xDomainDecomposition):
		...
		print("Skiploop: ",skipLoop,int(totalLocalDomains/size)*rank)
		...
		if ((domainCount >= skipLoop) or (domainCount < (int(totalLocalDomains/size)*rank))):
			...
			inCount = 0
			innerInCount = 0
			for i in range(0,nPointsInput):	# Determines which in mesh points in block
			...
			outCount = 0
			for i in range(0,nPointsOutput):# Determines which out mesh points in block
			...
			if (regularLocal == 1):
				...
				fr_regular_rank.append(fr_regular)
				regErrorLocalRegular = 0
				regErrorLocalRegular = pow(sum(pow(out_vals - fr_regular,2))/len(out_mesh),0.5)
				print("L2 Out Regular Local: ", regErrorLocalRegular)
				#error_LOOCV = LOOCV(bf, in_mesh, in_vals, rescale = False)
				#errorsLOOCV = error_LOOCV() 
				loocvErrorLocalRegular = 0
				#errorsLOOCV = error_LOOCV() 
				for k in range(0,len(errorsLOOCV)):
					loocvErrorLocalRegular += pow(errorsLOOCV[k],2)
				loocvErrorLocalRegular = loocvErrorLocalRegular/len(in_mesh)
				loocvErrorLocalRegular = pow(loocvErrorLocalRegular,0.5)
				print("L2 Error LOOCV Local: ", loocvErrorLocalRegular)
			else:	
			...
			for i in range(0,outCount):
				...
				out_vals_split_rational[out_mesh_list[i]] = fr[i]
				out_vals_split_regular[out_mesh_list[i]] = fr_regular[i]
				k += 1
...
## Calculate RMSE for the combined local domains into 1 domain

if (rank > 0):
	comm.Send([out_mesh_list_global], dest=0, tag=13)
if (rank == 0):
	comm.Recv([out_mesh_list_global], source=1, tag=13)

#for i in range(0,len(fr_regular)):
#	out_vals_split_regular[out_mesh_list_global[i]] = fr_regular[i]

out_vals_global = func(out_mesh_global[:,0],out_mesh_global[:,1])
out_vals_split_regularError = pow(sum(pow(out_vals_global - out_vals_split_regular,2))/len(out_mesh_global),0.5)

end = time.time()
print("Time for Partition of Unity RBF: ", end-start)
...
if (regularLocal == 1):
	print("Error of Local Regular RBF sub-domains: ", out_vals_split_regularError)
