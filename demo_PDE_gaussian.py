...
for j in range(0,nPoints):
		queryPt = (in_mesh[j,0],in_mesh[j,1])
		nnArray = tree.query(queryPt,10)
		gaussValue = 0
		gaussValueX.append(0)
		gaussValueY.append(0)
		#print(nnArray)
		for k in range(1,10):
			gaussValue += np.exp( -np.power(s * np.abs(nnArray[0][k]), 2))
			gaussValueX[j] += np.exp( -np.power(s * np.abs((in_mesh[nnArray[1][k],0] - in_mesh[nnArray[1][0],0])), 2))
			gaussValueY[j] += np.exp( -np.power(s * np.abs((in_mesh[nnArray[1][k],1] - in_mesh[nnArray[1][0],1])), 2))
		#nearest_neighbors.append(nnArray[0][1])
		shape_params.append(gaussValue/5)
...
surf = ax.scatter(in_mesh[:,0], in_mesh[:,1], shape_params, cmap=cm.jet, linewidth=0.1)
fig.colorbar(surf, shrink=0.5, aspect=5)
plt.show()

fig = plt.figure()
ax = Axes3D(fig)
surf = ax.plot_trisurf(in_mesh[:,0], in_mesh[:,1], gaussValueX, cmap=cm.jet, linewidth=0.1)
fig.colorbar(surf, shrink=0.5, aspect=5)
plt.show()

fig = plt.figure()
ax = Axes3D(fig)
surf = ax.plot_trisurf(in_mesh[:,0], in_mesh[:,1], gaussValueY, cmap=cm.jet, linewidth=0.1)
fig.colorbar(surf, shrink=0.5, aspect=5)
plt.show()
